...
model_xg_object <- unbundle(rf_bundle)

load("RF_XG_predict.RData")
model_ensemble_object <- unbundle(ensemble_bundle)
...
ensemble_mod <- fit_members(model_stack)
```
...
## Exporting to RData for future predictions
```{r, eval = FALSE}
...
ensemble_bundle <- bundle(ensemble_mod)
save(ensemble_bundle, file = "RF_XG_predict.RData")
```
